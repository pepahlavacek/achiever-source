...
Meteor.publish("postsFromTo", function(start, end) {
  return Posts.find( {
    $and : [
      { created_at: { $gt: start } },
      { created_at: { $lt: end } }
...
Meteor.publish('users', function() {
  return Meteor.users.find({}, { fields: { profile: 1, approved: 1, isAdmin: 1, createdAt: 1 }});
});

Meteor.publish('user', function(id) {
  return Meteor.users.find({ _id: this.userId }, { fields: { profile: 1, approved: 1, isAdmin: 1, createdAt: 1 }});
});

Meteor.publish("thisUser", function() {
  return Meteor.users.find({ _id: this.userId }, { fields: { roles: 1, profile: 1, approved: 1, isAdmin: 1 }});
});
